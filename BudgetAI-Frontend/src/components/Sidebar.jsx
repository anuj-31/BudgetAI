...
const Sidebar = ({activeMenu}) => {
    ...
    const displayName = user?.fullName?.trim() && /[a-zA-Z]/.test(user.fullName)
        ? user.fullName.trim()
    ...
    return (
        <div className="w-64 h-[calc(100vh-61px)] bg-white border-gray-200/50 p-5 sticky top-[61px] z-20">
            <div className="flex flex-col items-center justify-center gap-3 mt-3 mb-7">
                <div className="flex w-20 h-20 items-center justify-center rounded-full bg-purple-100 text-2xl font-semibold text-purple-700">
                    {initial}
                </div>
                <h5 className="text-gray-950 font-medium leading-6">{displayName}</h5>
            </div>
